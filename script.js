...
function setSize(size){
 selectedSize=size; selectedCount=allowedCount(); selectedFlavors=[];
 document.querySelectorAll(".size-chip,.size-option").forEach(x=>x.classList.toggle("active",x.dataset.size===size));
 document.getElementById("flavorRule").innerHTML=`<strong>${size}</strong> allows <strong>${selectedCount} flavor${selectedCount>1?"s":""}</strong>.`;
 document.getElementById("flavorCount").innerHTML=selectedCount===1?`<button class="count-btn active">1 FLAVOR</button>`:[1,2,3,4].filter(n=>n===selectedCount).map(n=>`<button class="count-btn active">${n} FLAVORS</button>`).join("");
 renderSelectors();
 renderSummary();
}
...
function renderSummary(){
 ...
 const price=valid.length?flavors[valid[0]][2][sizeKeys.indexOf(selectedSize)]:0;
...
function addPizza(){
 ...
 const price=flavors[valid[0]][2][sizeKeys.indexOf(selectedSize)];
